...
const string strMessageMagic = "DTCash Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
	...
	if (nBestHeight < 400000)
		nRewardCoinYear = 10 * COIN_YEAR_REWARD;
	else if (nBestHeight > 400000 && nBestHeight < 800000)
		nRewardCoinYear = 9 * COIN_YEAR_REWARD;
	else if (nBestHeight > 800000 && nBestHeight < 1200000)
		nRewardCoinYear = 8 * COIN_YEAR_REWARD;
	else if (nBestHeight > 1200000 && nBestHeight < 1600000)
		nRewardCoinYear = 7 * COIN_YEAR_REWARD;
	else if (nBestHeight > 1600000 && nBestHeight < 2000000)
		nRewardCoinYear = 6 * COIN_YEAR_REWARD;
	else if (nBestHeight > 2000000 && nBestHeight < 2400000)
		nRewardCoinYear = 5 * COIN_YEAR_REWARD;
	else if (nBestHeight > 2400000)
		nRewardCoinYear = 1 * COIN_YEAR_REWARD;


	int64_t nSubsidy = nCoinAge * nRewardCoinYear * 33 / (365 * 33 + 8);
	printf("nSubsidy Value %d \n", nSubsidy);
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
	...
	if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
	{
		...
		printf("*** %s\n", strMessage.c_str());
		uiInterface.ThreadSafeMessageBox(strMessage, "DTCash", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
		StartShutdown();
		return false;
	}
	return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	if (mapBlockIndex.empty())
	{
		...
		const char* pszTimestamp = "New Coin Start On 6 Sept 2018";
		CTransaction txNew;
		txNew.nTime = 1536215429;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
		...
		block.nVersion = 1;
		block.nTime = 1536215429;
		block.nBits = bnProofOfWorkLimit.GetCompact();
		block.nNonce = !fTestNet ? 2172051 : 2172051;
		...
		assert(block.hashMerkleRoot == uint256("0xdbecc906b5f0480a30a07bdb80c3d94a0a37c834f97a3ed8c1a4076828401e57"));
...
unsigned char pchMessageStart[4] = { 0x44, 0xe3, 0xd2, 0xe4 };
